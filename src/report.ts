...
const eventTimeWindow: number =
  new Config().minimumTimeIntervalBetweenEvents * 1000;
...
class Report {
  ...
  reportEvent(closeCall: CloseCall) {
    if (this.lastEvent != undefined) {
      if (
        closeCall.glider1 == this.lastEvent.glider1 &&
        closeCall.glider2 == this.lastEvent.glider2 &&
        Math.abs(closeCall.timestamp - this.lastEvent.timestamp) <=
          eventTimeWindow
      ) {
  ...
  generateHTMLReport() {
    const issueRows =
      this.loadIssues?.map(HTMLReport.createIssueRow).join("") ?? "";
    ...
    const htmlReport = HTMLReport.createHtml(
      issueTable,
      eventTable,
      shameTable,
      this.closeCalls.length
    );
